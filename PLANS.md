...
- Kept the app's PostgreSQL-backed consultation chat as the source of truth for medical messages. Built-in LiveKit chat remains a known limitation and should not be used for persisted consultation records.
...
- Manual QA confirmed the assigned doctor and owning patient can join the same LiveKit call, camera/microphone work, Leave works, the call is available only from consultation start through consultation end, completed/cancelled consultations reject joining, and admin/wrong patient/wrong doctor cannot access the call.
...
- Doctor schedule creation, patient-visible availability, and patient booking validation use one shared 5-minute minimum lead time.
...
- Patient-facing doctor profiles show only bookable `AVAILABLE` slots that start at least 5 minutes from the displayed current minute.
...
- Confirmed by inspection that future `AVAILABLE` doctor slots at least 5 minutes away are shown to patients and that past or too-soon `AVAILABLE` slots are hidden from patient booking.
