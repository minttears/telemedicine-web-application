...
| T708 | DONE | Add Daily video call UI | Phase 14C added dedicated patient/doctor call pages using Daily Prebuilt through `@daily-co/daily-js`. The existing call panel links to the role-scoped call page, the client requests a short-lived token only after join/start, and tokens are kept in memory only. Recording, transcription, screen sharing, persistent media storage, group calls, public links, raw WebRTC signaling, and admin call access remain excluded. |
...
| D002 | DEFERRED | Video call hardening | Daily call UI is implemented for the MVP. Full production device/network QA, call end persistence, recording, transcription, screen sharing, group calls, and admin call access remain deferred. |
