...
| T703 | DONE | Implement file attachment metadata and storage flow | First-version secure consultation file attachments are completed: patients and doctors can upload/download allowed files through server-mediated routes, blobs are stored in private Supabase Storage, and metadata is stored in PostgreSQL. True Supabase Realtime, video calls, legal prescription workflow, admin attachment/message access, virus scanning, advanced file previews, and production hardening remain TODO/deferred. |
